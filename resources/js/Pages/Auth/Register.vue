...
function switchTo(r) { active.value = r; form.role = r }
function submit() { form.post('/register') } // <-- directo, sin Ziggy
</script>
  ...
  <div class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
    <div class="w-full max-w-md bg-white dark:bg-gray-800 shadow rounded-2xl p-6">
      <div class="flex mb-4 rounded-xl overflow-hidden">
        <button @click="switchTo('student')"  class="flex-1 py-2 text-center" :class="active==='student'  ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-700 dark:text-gray-200'">Alumno</button>
        <button @click="switchTo('company')"  class="flex-1 py-2 text-center" :class="active==='company'  ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-700 dark:text-gray-200'">Empresa</button>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-4">
        ...
        <p class="text-center text-sm text-gray-500 dark:text-gray-400">
          ¿Ya tienes cuenta?
          <Link href="/login" class="text-indigo-600 hover:underline">Entrar</Link>  <!-- directo -->
        </p>
      </form>
    </div>
  </div>
