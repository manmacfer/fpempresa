...
function addItem(field, valueField=null){ const val=(valueField?form[valueField]:form[field])?.toString().trim(); if(!val) return; form[field].push(val); if(valueField) form[valueField]=''; else form[field]='' }
function removeItem(field, i){ form[field].splice(i,1) }

function addLanguage(){ const n=form.newLangName?.toString().trim(); const l=form.newLangLevel?.toString().trim(); if(!n) return; form.languages.push({ name:n, level:l }); form.newLangName=''; form.newLangLevel='' }
function removeLanguage(i){ form.languages.splice(i,1) }

function onPickAvatar(e){ const f=e.target.files?.[0]; form.avatar=f||null; if(f){ const r=new FileReader(); r.onload=()=>previewUrl.value=r.result; r.readAsDataURL(f) } }
function onPickOtherCerts(e){ form.other_certs = Array.from(e.target.files || []) }
...
function openEduEdit(e){ editingEdu.value=e; eduForm.reset(); eduForm.clearErrors(); eduForm.title=e.title||''; eduForm.center=e.center||''; eduForm.start_date=e.start_date||''; eduForm.end_date=e.end_date||''; showEduModal.value=true }
...
function openExpEdit(x){ editingExp.value=x; expForm.reset(); expForm.clearErrors(); expForm.company=x.company||''; expForm.position=x.position||''; expForm.start_date=x.start_date||''; expForm.end_date=x.end_date||''; expForm.functions=x.functions||''; expForm.is_non_formal=!!x.is_non_formal; showExpModal.value=true }
  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <Link v-if="props.student.id" :href="route('students.public.show', props.student.id)" class="text-sm text-indigo-600 hover:underline dark:text-indigo-400">Ver perfil público</Link>
      </div>
    </template>
    ...
    <div class="mx-auto max-w-5xl p-6 space-y-8">
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          <div class="grid grid-cols-1 gap-3">
            <div>
              ...
              <input type="file" accept=".pdf,.doc,.docx" @change="e=> form.cv = e.target.files?.[0] || null" class="text-sm" />
            </div>
            <div>
              <label class="block text-sm text-gray-600 dark:text-gray-300 mb-1">Carta de presentación (opcional)</label>
              <input type="file" accept=".pdf,.doc,.docx" @change="e=> form.cover_letter = e.target.files?.[0] || null" class="text-sm" />
            </div>
            <div>
              <label class="block text-sm text-gray-600 dark:text-gray-300 mb-1">Otros certificados (múltiples)</label>
              <input type="file" multiple accept=".pdf,.jpg,.jpeg,.png" @change="onPickOtherCerts" class="text-sm" />
            </div>
          </div>
        </div>
        <div class="mt-2 text-xs text-red-600">{{ form.errors.avatar || form.errors.cv || form.errors.cover_letter || form.errors['other_certs.*'] }}</div>
      </section>
      ...
      <section class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div class="flex items-center gap-6 sm:col-span-2">
            <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300"><input v-model="form.has_driver_license" type="checkbox" class="rounded border-gray-300 dark:border-gray-700"> Carnet de conducir</label>
            <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300"><input v-model="form.has_vehicle" type="checkbox" class="rounded border-gray-300 dark:border-gray-700"> Vehículo propio</label>
          </div>
        </div>
      </section>
      ...
      <section class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            ...
            <select v-model="form.cycle" class="w-full rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100">
          ...
          <div class="sm:col-span-2">
            ...
            <div class="flex items-center gap-4">
              <label class="inline-flex items-center gap-2 text-sm"><input v-model="form.fp_modality" type="radio" value="dual"> Dual</label>
              <label class="inline-flex items-center gap-2 text-sm"><input v-model="form.fp_modality" type="radio" value="general"> General</label>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div v-if="form.work_modality==='hibrida'">
            ...
            <input v-model="form.remote_days" type="number" min="0" max="7" class="w-full rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"/>
          </div>
          <div>
          ...
          <div>
            ...
            <input v-model="form.available_from" type="date" class="w-full rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"/>
          </div>
          <div class="flex items-center gap-6 sm:col-span-2">
            <label class="inline-flex items-center gap-2 text-sm"><input v-model="form.transport_own" type="checkbox"> Transporte propio</label>
            <label class="inline-flex items-center gap-2 text-sm"><input v-model="form.relocate" type="checkbox"> Puedo desplazarme</label>
          </div>
          <div class="sm:col-span-2">
            <label class="block text-sm text-gray-600 dark:text-gray-300 mb-1">Ciudades a las que puedes desplazarte</label>
            <div class="flex flex-wrap gap-2">
              <input v-model="form.newRelocateCity" type="text" placeholder="Ciudad" class="w-48 rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" @keyup.enter="addItem('relocate_cities','newRelocateCity')"/>
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              <span v-for="(c,i) in form.relocate_cities" :key="i" class="rounded-full bg-gray-100 px-3 py-1 text-xs dark:bg-gray-800">{{ c }} <button class="ms-2 text-gray-500" @click="removeItem('relocate_cities', i)">✕</button></span>
            </div>
          </div>
          <div class="sm:col-span-2">
            <label class="block text-sm text-gray-600 dark:text-gray-300 mb-1">Compromisos que afecten tu disponibilidad</label>
            <div class="flex flex-wrap gap-2">
              <input v-model="form.newCommitment" type="text" placeholder="Ej. Trabajo parcial" class="w-64 rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" @keyup.enter="addItem('commitments','newCommitment')"/>
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              <span v-for="(c,i) in form.commitments" :key="i" class="rounded-full bg-gray-100 px-3 py-1 text-xs dark:bg-gray-800">{{ c }} <button class="ms-2 text-gray-500" @click="removeItem('commitments', i)">✕</button></span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div class="sm:col-span-2">
            ...
            <div class="flex flex-wrap gap-2">
              <input v-model="form.newSector" type="text" placeholder="Ej. Desarrollo web" class="w-64 rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" @keyup.enter="addItem('sectors','newSector')"/>
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              <span v-for="(s,i) in form.sectors" :key="i" class="rounded-full bg-gray-100 px-3 py-1 text-xs dark:bg-gray-800">{{ s }} <button class="ms-2 text-gray-500" @click="removeItem('sectors', i)">✕</button></span>
            </div>
          </div>
          <div>
            <label class="block text-sm text-gray-600 dark:text-gray-300 mb-1">Tipo de empresa preferida</label>
            <input v-model="form.preferred_company_type" type="text" class="w-full rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"/>
          </div>
          <div>
            <label class="block text-sm text-gray-600 dark:text-gray-300 mb-1">Experiencia no formal</label>
            <input v-model="form.non_formal_experience" type="text" class="w-full rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"/>
          </div>
          <div class="sm:col-span-2">
            <label class="block text-sm text-gray-600 dark:text-gray-300 mb-1">Competencias técnicas</label>
            <div class="flex flex-wrap gap-2">
              <input v-model="form.newTech" type="text" placeholder="Ej. Laravel" class="w-56 rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" @keyup.enter="addItem('tech_competencies','newTech')"/>
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              <span v-for="(t,i) in form.tech_competencies" :key="i" class="rounded-full bg-gray-100 px-3 py-1 text-xs dark:bg-gray-800">{{ t }} <button class="ms-2 text-gray-500" @click="removeItem('tech_competencies', i)">✕</button></span>
            </div>
          </div>
          <div class="sm:col-span-2">
            <label class="block text-sm text-gray-600 dark:text-gray-300 mb-1">Habilidades personales</label>
            <div class="flex flex-wrap gap-2">
              <input v-model="form.newSoft" type="text" placeholder="Ej. Trabajo en equipo" class="w-56 rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" @keyup.enter="addItem('soft_skills','newSoft')"/>
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              <span v-for="(s,i) in form.soft_skills" :key="i" class="rounded-full bg-gray-100 px-3 py-1 text-xs dark:bg-gray-800">{{ s }} <button class="ms-2 text-gray-500" @click="removeItem('soft_skills', i)">✕</button></span>
            </div>
          </div>
          <div class="sm:col-span-2">
            <label class="block text-sm text-gray-600 dark:text-gray-300 mb-1">Idiomas</label>
            <div class="flex flex-wrap items-center gap-2">
              <input v-model="form.newLangName" type="text" placeholder="Idioma" class="w-40 rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"/>
              <input v-model="form.newLangLevel" type="text" placeholder="Nivel (A2, B1, C1…)" class="w-40 rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"/>
              <button type="button" @click="addLanguage" class="rounded-lg bg-gray-900 px-3 py-2 text-sm text-white dark:bg-gray-100 dark:text-gray-900">Añadir</button>
            </div>
            <ul class="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
              <li v-for="(lng,i) in form.languages" :key="i" class="flex items-center justify-between rounded-lg bg-gray-50 px-3 py-2 text-sm dark:bg-gray-800/60">
                <span class="truncate"><strong>{{ lng.name }}</strong><span v-if="lng.level"> · {{ lng.level }}</span></span>
                <button type="button" @click="removeLanguage(i)" class="text-xs text-gray-500 hover:text-red-600">Eliminar</button>
              </li>
            </ul>
          </div>
          <div class="sm:col-span-2">
            ...
            <div class="flex flex-wrap gap-2">
              <input v-model="form.newCert" type="text" placeholder="Ej. Cisco CCNA" class="w-56 rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" @keyup.enter="addItem('certifications','newCert')"/>
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              <span v-for="(c,i) in form.certifications" :key="i" class="rounded-full bg-gray-100 px-3 py-1 text-xs dark:bg-gray-800">{{ c }} <button class="ms-2 text-gray-500" @click="removeItem('certifications', i)">✕</button></span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
        <div class="mb-3 flex items-center justify-between">
          ...
          <button type="button" @click="openEduCreate" class="rounded-lg bg-gray-900 px-3 py-2 text-sm text-white dark:bg-gray-100 dark:text-gray-900">Añadir</button>
        </div>
        <div v-if="props.student.educations?.length" class="space-y-2">
          <div v-for="e in props.student.educations" :key="e.id" class="flex items-center justify-between rounded-lg bg-gray-50 p-3 text-sm dark:bg-gray-800/60">
            <div>
              <div class="font-medium text-gray-800 dark:text-gray-100">{{ e.title }}</div>
              <div class="text-gray-500 dark:text-gray-400"><span v-if="e.center">{{ e.center }} · </span>{{ e.start_date || '—' }} — {{ e.end_date || 'Actual' }}</div>
            </div>
            <div class="flex items-center gap-2">
              <button type="button" @click="openEduEdit(e)" class="rounded-md px-2 py-1 text-xs text-indigo-600 hover:bg-indigo-50 dark:text-indigo-400 dark:hover:bg-indigo-900/20">Editar</button>
              <button type="button" @click="deleteEducation(e)" class="rounded-md px-2 py-1 text-xs text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">Eliminar</button>
            </div>
          </div>
        </div>
      ...
      <section class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
        <div class="mb-3 flex items-center justify-between">
          ...
          <button type="button" @click="openExpCreate" class="rounded-lg bg-gray-900 px-3 py-2 text-sm text-white dark:bg-gray-100 dark:text-gray-900">Añadir</button>
        </div>
        <div v-if="props.student.experiences?.length" class="space-y-2">
          <div v-for="x in props.student.experiences" :key="x.id" class="flex items-center justify-between rounded-lg bg-gray-50 p-3 text-sm dark:bg-gray-800/60">
            <div>
              ...
              <div class="text-gray-500 dark:text-gray-400">{{ x.start_date || '—' }} — {{ x.end_date || 'Actual' }}<span v-if="x.is_non_formal" class="ms-2 rounded bg-gray-200 px-2 py-0.5 text-[11px] dark:bg-gray-700">No formal</span></div>
              <div v-if="x.functions" class="text-gray-700 dark:text-gray-300">{{ x.functions }}</div>
            </div>
            <div class="flex items-center gap-2">
              <button type="button" @click="openExpEdit(x)" class="rounded-md px-2 py-1 text-xs text-indigo-600 hover:bg-indigo-50 dark:text-indigo-400 dark:hover:bg-indigo-900/20">Editar</button>
              <button type="button" @click="deleteExperience(x)" class="rounded-md px-2 py-1 text-xs text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">Eliminar</button>
            </div>
          </div>
        </div>
      ...
      <div class="flex items-center justify-end gap-3">
        ...
        <button type="submit" @click="submit" :disabled="form.processing" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-50 dark:bg-indigo-500">Guardar</button>
      </div>
    </div>
    ...
    <div v-if="showEduModal" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative w-full max-w-lg rounded-2xl border border-gray-200 bg-white p-5 shadow-xl dark:border-gray-800 dark:bg-gray-900">
        <h3 class="mb-4 text-base font-semibold text-gray-900 dark:text-gray-100">{{ editingEdu ? 'Editar formación' : 'Añadir formación' }}</h3>
        <div class="grid grid-cols-1 gap-3">
          <div>
            <label class="mb-1 block text-sm text-gray-600 dark:text-gray-300">Título</label>
            <input v-model="eduForm.title" type="text" class="w-full rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"/>
        ...
        <div class="mt-5 flex items-center justify-end gap-2">
          <button type="button" @click="closeEdu" class="rounded-lg px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800">Cancelar</button>
          <button type="button" @click="saveEducation" :disabled="eduForm.processing" class="rounded-lg bg-indigo-600 px-3 py-2 text-sm text-white disabled:opacity-50 dark:bg-indigo-500">Guardar</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showExpModal" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative w-full max-w-xl rounded-2xl border border-gray-200 bg-white p-5 shadow-xl dark:border-gray-800 dark:bg-gray-900">
        <h3 class="mb-4 text-base font-semibold text-gray-900 dark:text-gray-100">{{ editingExp ? 'Editar experiencia' : 'Añadir experiencia' }}</h3>
        <div class="grid grid-cols-1 gap-3">
          <div>
            <label class="mb-1 block text-sm text-gray-600 dark:text-gray-300">Empresa</label>
            <input v-model="expForm.company" type="text" class="w-full rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"/>
          ...
          <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
            <input v-model="expForm.is_non_formal" type="checkbox" class="rounded border-gray-300 dark:border-gray-700"> Experiencia no formal
          </label>
        </div>
        <div class="mt-5 flex items-center justify-end gap-2">
          <button type="button" @click="closeExp" class="rounded-lg px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800">Cancelar</button>
          <button type="button" @click="saveExperience" :disabled="expForm.processing" class="rounded-lg bg-indigo-600 px-3 py-2 text-sm text-white disabled:opacity-50 dark:bg-indigo-500">Guardar</button>
        </div>
      </div>
    </div>
  </AuthenticatedLayout>
